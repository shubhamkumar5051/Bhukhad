...
if(menuList?.cards?.[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards?.[2]?.card?.card?.itemCards)
{
  fetchItemCards =menuList?.cards?.[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards?.[2]?.card
    ?.card?.itemCards;
}else{
  //sweets corner ke liye dimag lagana padega
  fetchItemCards= menuList?.cards?.[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards?.[2]?.card?.card?.categories[0].itemCards;
 ...
 return (
    <div className="text-center ">
      ...
      categories.map((category)=>{
        return (<RestroCategory data={category.card.card}/>)
      })
      }
